...
def learn_embeddings(walks, args):
    ...
    '''
    walks = [list(map(str, walk)) for walk in walks]  # Convert map object to list
...
def main(args):
    ...
    # Use Node2Vec from the node2vec package
    node2vec = Node2Vec(nx_G, dimensions=args.dimensions, walk_length=args.walk_length,
                        num_walks=args.num_walks, workers=args.workers, p=args.p, q=args.q)
    
    walks = node2vec.walks  # Simulated random walks
